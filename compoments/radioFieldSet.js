    ...
    if (value) {
      ...
      if (selectedOption.conditional) {
        if (this.conditionalValue[value]) {
          if (
            selectedOption.conditional.validator &&
            !selectedOption.conditional.validator.isValid(
              this.conditionalValue[value]
            )
          ) {
            return this.setState({
              error: selectedOption.conditional.validator.errorMessage,
            })
          }
  ...
  render() {
    return (
      <div>
              ...
              <div
                className={
                  this.conditional
                    ...
                    : 'govuk-radios lbh-radios'
                }
              >
                {this.options.map((o, i) => (
                  <span key={i}>
                    {this.includeOrDivider(i) ? (
                      <div className="govuk-radios__divider">or</div>
                    ) : (
                      <div className="govuk-!-margin-bottom-2"></div>
                    )}
                    <div className="govuk-radios__item">
                      ...
                      />
                      <label
                        className="govuk-label govuk-radios__label lbh-label"
                        htmlFor={`${this.name}-${i}`}
                      >
                        {o.title}
                      </label>
                    ...
                    {o.conditional && (
                      <div
                        className={`govuk-radios__conditional ${
                          this.state.value[this.name] != o.value &&
                      ...
                      >
                        ...
                        >
                          <label
                            ...
                            htmlFor={`${this.name}-${o.value}`}
                          >
                            {o.conditional.label}
                          </label>
                          <input
                            className="govuk-input govuk-!-width-one-third"
                            id={`${this.name}-${o.value}`}
                            name={`${this.name}-${o.value}`}
        ...
        <div>
          <div className="govuk-!-margin-top-6">{this.beforeButton}</div>
